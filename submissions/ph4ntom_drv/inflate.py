...
class JointFrameGenerator(nn.Module):
    def __init__(self, num_classes=5, pose_dim=6, cond_dim=64, depth_mult=1):
        ...
        self.shared_trunk = SharedMaskDecoder(
            num_classes=num_classes, emb_dim=6, c1=64, c2=80, depth_mult=depth_mult)

        self.pose_mlp = nn.Sequential(
            nn.Linear(pose_dim, cond_dim), nn.SiLU(), nn.Linear(cond_dim, cond_dim))

        self.frame1_head = FrameHead(
            in_ch=64, cond_dim=cond_dim, hidden=60, depth_mult=depth_mult)

        self.frame2_head = Frame2StaticHead(
            in_ch=64, hidden=60, depth_mult=depth_mult)
